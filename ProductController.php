...
class ProductControllerCore extends ProductPresentingFrontControllerCore
{
    ...
    public function initContent()
    {
        if (!$this->errors) {
			...
			/*Product category*/
			$product_categories = Product::getProductCategoriesFull($this->product->id);
			$product_subcategories = Product::getProductCategoriesFull($this->product->id);
			
			/*All categories for product page ... 16- id of root category*/
			$allCategories = Category::getNestedCategories(16, $this->context->language->id);
			
			$myCustomCatIDs = array(16,11,12,13);
			$myCustomCats = Category::getCategoryInformations( $myCustomCatIDs );

            
			$this->context->smarty->assign(array(
			'myCustomCats'=>$myCustomCats,
